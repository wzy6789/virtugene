 ...
 * 手机端已迁移至独立项目（virtugene-mobile），此仓库仅桌面端。
 */
...
const noop: VirtuGeneAPI = {
  ...
  sync: {
    start: async () => ({ ok: false, error: '未在 Electron 环境中运行' }),
    stop: async () => ({ ok: false }),
    status: async () => ({ running: false, port: 0, url: '', addresses: [] }),
    setExportData: async () => ({ ok: false }),
